...
def write_english_ass(timing: dict) -> Path:
    sentences = timing["sentences"]

    lines = [
        "[Script Info]",
        "Title: Poppi English Subtitles",
        "ScriptType: v4.00+",
        "PlayResX: 1920",
        "PlayResY: 1080",
        "WrapStyle: 0",
        ...
        "Style: English,Arial,42,&H00FFFFFF,&H00FFFFFF,&H00000000,&H70000000,0,0,0,0,100,100,0,0,1,2.4,0.6,2,125,125,54,1",
        "",
        "[Events]",
    ...
    for sentence in sentences:
        ...
        lines.append(
            f"Dialogue: 0,{ass_time(sentence['start'])},{ass_time(sentence['end'])},English,,0,0,0,,{english}"
        )
    path = SUBTITLES / "subtitles_en.ass"
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    return path
...
def main() -> None:
    ...
    ass_path = write_english_ass(timing)
    ...
    burn_subtitles(no_subtitles, ass_path, final)
    print(no_subtitles)
    print(final)
